...
def enforce_topdown_rule_indexes(dv: DecisionVariables):
    return [
        ...
        all(dv.topdown_rule_indexes[n][rule] == (dv.max_depth+1))
        ...
        (min([
            dv.max_depth+1, 
            abs( dv.rule[n]                 - rule) * (dv.max_depth+1) + dv.depth[n]
            ]+[
            (abs(dv.ancestor_rule[n, depth] - rule) * (dv.max_depth+1) + depth
            ) for depth in range(dv.max_depth)]

        ) == dv.topdown_rule_indexes[n][rule][0])
        ...
        (dv.topdown_rule_indexes[n][rule][occurence-1] + 1 < dv.max_depth    ) | 
        (dv.topdown_rule_indexes[n][rule][occurence  ]    == dv.max_depth + 1)
